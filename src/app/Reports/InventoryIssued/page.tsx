  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        ...
        const uniqueNames = Array.from(new Set(reportData.map((r) => r.Name)));
setAllNames(["All", ...uniqueNames]);



        setReports(reportData);
        setFilteredReports(reportData); // Initially show all
      } catch (err: any) {
        console.error(err);
        setError(err.message);
      } finally {
  ...
  const to = new Date(toDate);
    ...
    return dateInRange && nameMatch;
  });

  setFilteredReports(filtered);
}, [fromDate, toDate, selectedName, reports]);
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {items.map((report, index) => (
                      <tr key={`${report.Id}-${index}`} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 text-sm text-gray-800">
                          {report.PureMetalweight !== undefined && report.PureMetalweight !== null ? report.PureMetalweight : "-"}
                        </td>
                        <td className="px-4 py-2 text-sm text-gray-800">
                          {report.AlloyWeight !== undefined && report.AlloyWeight !== null ? report.AlloyWeight : "-"}
                        </td>
                        <td className="px-4 py-2 text-sm text-gray-800">
                          {report.CreatedDate
